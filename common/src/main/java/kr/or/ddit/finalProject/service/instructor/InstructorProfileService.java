 ...
 *   - 프로필 이미지 업로드 (Cloudinary → INSTRUCTOR.INSTR_PROFILE_IMG)
 *   - 프로필 이미지 제거 (INSTRUCTOR.INSTR_PROFILE_IMG = NULL)
 *   - 소개글 수정 (INSTRUCTOR.INSTR_INTRO, 빈 문자열 → null 처리)
 *   - 약력 항목 조회 / 등록 / 수정 / 소프트 딜리트 (INSTRUCTOR_CAREER)
 *
 * [관련 URL]
 *   GET  /instructor/profile/teacher
 *   POST /instructor/profile/teacher/image
 *   POST /instructor/profile/teacher/image/delete
 *   POST /instructor/profile/teacher/intro
 *   POST /instructor/profile/teacher/careers
 *   POST /instructor/profile/teacher/careers/{sn}/update
 *   POST /instructor/profile/teacher/careers/{sn}/delete
 */
public interface InstructorProfileService {

    /**
     * 강사 기본 프로필 조회
     * 소개글(instrIntro)과 프로필 이미지 URL(instrProfileImg)을 반환합니다.
     *
    ...
    /**
     * 강사의 약력 항목 전체 조회
     * DEL_YN = 'N'인 항목만 반환합니다 (소프트 딜리트 적용).
     * 유형(careerTypeCd) → 순서(sortOrd) → 시작연도(careerStrtYr) 순으로 반환됩니다.
     *
     * @param instrUserId 조회할 강사 ID
     * @return 약력 항목 목록
     */
    ...
    /**
     * 프로필 이미지 업로드 및 저장
     * Cloudinary에 이미지를 업로드하고, 반환된 URL을 INSTRUCTOR 테이블에 저장합니다.
     *
    ...
    /**
     * 프로필 이미지 제거
     * INSTRUCTOR.INSTR_PROFILE_IMG를 NULL로 업데이트합니다.
     *
     * @param instrUserId 대상 강사 ID
     */
    ...
    /**
     * 강사 소개글 수정
     * 빈 문자열로 들어오면 null로 변환하여 저장합니다 (소개글 삭제 효과).
     *
     * @param instrUserId 대상 강사 ID
    ...
    /**
     * 약력 항목 등록
     * careerEndYr가 빈 문자열로 들어오는 경우 null로 변환하여 저장합니다.
     * RGTR_ID(등록자)를 instrUserId로 설정합니다.
     *
    ...
    /**
     * 약력 항목 수정
     * 소유권 확인 후 수정합니다 (다른 강사의 항목 수정 불가).
     * LAST_MDFR_ID(수정자)를 instrUserId로 설정합니다.
     *
    ...
    /**
     * 약력 항목 소프트 딜리트
     * 소유권 확인 후 DEL_YN = 'Y'로 마킹합니다 (물리 삭제 없음).
     *
